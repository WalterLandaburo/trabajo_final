...
def buscar_producto():
    ...
    print("Si desea buscar por Si desea buscar un producto por su ID, ingrese 'ID'.\nSimdesea buscar un producto por su nombre, Ingrese 'N'.\nSi desea buscar un producto por su de categoría, ingrese 'C'.").strip()
    ...
    #Buscar por ID    
    if buscar == "id":
        cursor.execute('SELECT id, nombre, descripcion, cantidad, precio, categoria FROM productos WHERE id = ?', (buscar,))
        productos = cursor.fetchall()
        
        if not productos:
            print(Fore.YELLOW + "⚠️ No se encontro un producto que coincida con el dato ingresado.")
        
        print(Fore.GREEN + f"=== \nProductos que coincidan con {buscar} ===")
        for p in productos:
            print(f"ID: {p[0]}, Nombre: {p[1]}, Descripción: {p[2]}, Cantidad: {p[3]}, Precio: ${p[4]:.2f}, Categoría: {p[5]}")
    ...
    #Buscar por nombre    
    if buscar == "n":
        cursor.execute('SELECT id, nombre, descripcion, cantidad, precio, categoria FROM productos WHERE nombre = ?', (buscar,))
        productos = cursor.fetchall()
    ...
    if buscar == "c":
        cursor.execute('SELECT id, nombre, descripcion, cantidad, precio, categoria FROM productos WHERE categoria = ?', (buscar,))
        productos = cursor.fetchall()
...
def menu():
    ...
    while True:
        ...
        if opcion == "2":
            if not productos:
                print(Fore.YELLOW + "⚠️ No hay productos en la tabla.")
